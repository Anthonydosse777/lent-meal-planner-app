...
import {
    ...
    lastNDates, formatDate, todayDate, logMeal, getFoodHistory, recordFoodHistory,
    type LoggedMeal, type WeightEntry, type FoodHistoryItem,
} from "../../lib/storage";
...
function notifySaveError(e: unknown) {
    const msg = e instanceof Error ? e.message : "Something went wrong saving your data.";
    if (Platform.OS === "web") window.alert(msg);
    else Alert.alert("Save failed", msg);
}

const UNITS = ["g", "oz", "lb", "ml", "cup", "tbsp", "tsp"] as const;
...
function scoreRelevance(food: USDAFood, query: string): number {
    ...
    // Prefer whole foods over branded products
...
export default function ProgressScreen() {
    ...
    // Food items logger
    const [foodItems, setFoodItems] = useState<{ name: string; amount: string; unit: FoodUnit; nutrition: { calories: number; protein: number; carbs: number; fat: number; fiber: number } | null }[]>([]);
    const [showFoodForm, setShowFoodForm] = useState(false);
    const [foodSearch, setFoodSearch] = useState("");
    const [selectedFoodIdx, setSelectedFoodIdx] = useState<number | null>(null);
    const [unitMenu, setUnitMenu] = useState<{ idx: number; x: number; y: number } | null>(null);
    const unitButtonRefs = useRef<(View | null)[]>([]);
        ...
        searchTimer.current = setTimeout(async () => {
            try {
                const [usdaRes, nxRes, offRes] = await Promise.all([
                    ...
                    searchFoodsNutritionix(foodSearch).catch(() => [] as USDAFood[]),
                    searchFoodsOFF(foodSearch).catch(() => [] as USDAFood[]),
                ]);
                ...
                merged.sort((a, b) => scoreRelevance(b, foodSearch) - scoreRelevance(a, foodSearch));
                setUsdaResults(merged);
            } catch {
                setUsdaResults([]);
            } finally {
    ...
    // Load food history whenever the food form opens
    useEffect(() => {
        if (showFoodForm) {
    ...
    function addFoodItem(food: USDAFood) {
        setFoodItems([...foodItems, { name: food.description, amount: "", unit: "g", nutrition: food.per100g }]);
        setFoodSearch("");
        setUsdaResults([]);
        setSelectedFoodIdx(null);
        recordFoodHistory(food.description, food.brandName, food.per100g);
        getFoodHistory().then(setFoodHistory);
    }
    ...
    function addCustomFoodItem() {
        ...
        setFoodItems([...foodItems, { name, amount: "", unit: "g", nutrition: null }]);
        setFoodSearch("");
        setUsdaResults([]);
    }

    function updateFoodAmount(idx: number, amount: string) {
        const updated = [...foodItems];
        updated[idx].amount = amount;
        setFoodItems(updated);
    }

    function openUnitMenu(idx: number) {
        const ref = unitButtonRefs.current[idx];
        ref?.measureInWindow((x, y, _w, h) => {
            setUnitMenu({ idx, x, y: y + h + 4 });
        });
    }

    function setItemUnit(idx: number, unit: FoodUnit) {
        const updated = [...foodItems];
    ...
    function getFoodItemNutrition(item: typeof foodItems[0]) {
        const grams = (parseFloat(item.amount) || 0) * UNIT_TO_GRAMS[item.unit];
        if (!item.nutrition || grams <= 0) return { calories: 0, protein: 0, carbs: 0, fat: 0, fiber: 0 };
    ...
    async function handleSaveFoodItems() {
        ...
        const validItems = foodItems.filter((item) => parseFloat(item.amount) > 0);
        if (validItems.length === 0) return;
        setSavingMeal(true);
        const title = validItems.map((item) => `${item.name} ${item.amount}${item.unit}`).join(", ");
        try {
            await logMeal({
                id: Math.random().toString(36).slice(2, 10),
                title,
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: C.background }}>
                        ...
                        <View style={{
                            ...
                            {showFoodForm && (
                                <View style={{ marginTop: 16 }}>
                                    ...
                                    {foodSearch.trim().length === 0 && foodHistory.length > 0 && (
                                        <View style={{
                                            backgroundColor: C.cardElevated,
                                            ...
                                            <View style={{
                                                ...
                                                <MaterialCommunityIcons name="history" size={13} color={C.textDim} />
                                                <Text style={{ color: C.textDim, fontSize: 11, fontWeight: "700", letterSpacing: 0.4 }}>
                                                    RECENT FOODS
                                                </Text>
                                            </View>
                                            {foodHistory.slice(0, 8).map((item, i) => (
                                                <TouchableOpacity
                                                    key={item.name}
                                                    onPress={() => addFoodItem({
                                                        fdcId: -(1000 + i),
                                                    ...
                                                    style={{
                                                        ...
                                                        borderBottomWidth: i < Math.min(foodHistory.length, 8) - 1 ? 1 : 0,
                                                ...
                                                >
                                                    <View style={{ flex: 1 }}>
                                                        ...
                                                        {item.brandName ? (
                                                            <Text style={{ color: C.textDim, fontSize: 11, marginTop: 1 }} numberOfLines={1}>
                                                                {item.brandName}
                                                            </Text>
                                                        ) : null}
                                                        <Text style={{ color: C.textDim, fontSize: 11, marginTop: 2 }}>
                                                            {item.per100g.calories} cal · {item.per100g.protein}g P · {item.per100g.carbs}g C · {item.per100g.fat}g F / 100g
                                                        </Text>
                                                    </View>
                                                    <View style={{
                                                        backgroundColor: C.accentMuted,
                                                        ...
                                                        <Text style={{ color: C.accent, fontSize: 10, fontWeight: "700" }}>×{item.count}</Text>
                                                    </View>
                                                </TouchableOpacity>
                                            ))}
                                        </View>
                                    ...
                                    {foodItems.length > 0 && (
                                        <View style={{ marginTop: 14, gap: 8 }}>
                                                ...
                                                return (
                                                    <View key={idx} style={{
                                                        ...
                                                        <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                                                            ...
                                                            <TextInput
                                                                value={item.amount}
                                                                onChangeText={(v) => updateFoodAmount(idx, v)}
                                                                placeholder="amount"
                                                                placeholderTextColor={C.textDim}
                                                                keyboardType="decimal-pad"
                                                                style={{
                                                            ...
                                                            />
                                                            <View ref={(r) => { unitButtonRefs.current[idx] = r; }}>
                                                                <TouchableOpacity
                                                                    onPress={() => openUnitMenu(idx)}
                                                                    style={{
                                                                        backgroundColor: C.card,
                                                                        borderWidth: 1.5,
                                                                        borderColor: unitMenu?.idx === idx ? C.accent : C.border,
                                                                        borderRadius: 10, paddingHorizontal: 8,
                                                                        paddingVertical: 6, minWidth: 54,
                                                                        flexDirection: "row", alignItems: "center", gap: 4,
                                                                    }}
                                                                >
                                                                    <Text style={{ color: C.accent, fontSize: 13, fontWeight: "700", flex: 1, textAlign: "center" }}>{item.unit}</Text>
                                                                    <Text style={{ color: C.textDim, fontSize: 9 }}>{unitMenu?.idx === idx ? "▲" : "▼"}</Text>
                                                                </TouchableOpacity>
                                                            </View>
                                                        ...
                                                        {item.nutrition && parseFloat(item.amount) > 0 && (
                                                            <View style={{
                                            ...
                                            <TouchableOpacity
                                                ...
                                                disabled={savingMeal || foodItems.every((i) => !parseFloat(i.amount))}
                                                activeOpacity={0.7}
                                                style={{
                                                    paddingVertical: 13, borderRadius: 14,
                                                    backgroundColor: foodItems.some((i) => parseFloat(i.amount) > 0) ? C.accent : C.cardElevated,
                                                    alignItems: "center",
                                                }}
                                            >
                                                <Text style={{
                                                    color: foodItems.some((i) => parseFloat(i.amount) > 0) ? C.background : C.textDim,
                                                    fontWeight: "800", fontSize: 14,
                                                }}>
            ...
            <Modal
                visible={unitMenu !== null}
                transparent
                animationType="none"
                onRequestClose={() => setUnitMenu(null)}
            >
                <TouchableOpacity
                    style={{ flex: 1 }}
                    activeOpacity={1}
                    onPress={() => setUnitMenu(null)}
                />
                {unitMenu !== null && (
                    <View style={{
                        position: "absolute",
                        ...
                        {UNITS.map((u) => (
                            <TouchableOpacity
                                key={u}
                                onPress={() => setItemUnit(unitMenu.idx, u)}
                                style={{
                                    paddingHorizontal: 18,
                                    paddingVertical: 12,
                                    backgroundColor: u === foodItems[unitMenu.idx]?.unit ? C.accentMuted : "transparent",
                                }}
                            >
                                <Text style={{
                                    color: u === foodItems[unitMenu.idx]?.unit ? C.accent : C.text,
                                    fontWeight: u === foodItems[unitMenu.idx]?.unit ? "700" : "400",
                                    fontSize: 14,
                                }}>{u}</Text>
                            </TouchableOpacity>
                        ))}
