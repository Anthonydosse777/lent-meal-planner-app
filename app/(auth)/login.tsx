...
function friendlyAuthError(rawMessage: string, mode: 'login' | 'signup'): string {
    const lower = rawMessage.toLowerCase();
    if (lower.includes('invalid login') || lower.includes('invalid credentials')) {
        return "Incorrect email or password. Please try again.";
    }
    if (lower.includes('email not confirmed')) {
        return "Your email isn't verified yet. Please check your inbox for the confirmation link.";
    }
    ...
    async function handleSubmit() {
        ...
        try {
            if (mode === 'login') {
                ...
                if (error) setAuthError(friendlyAuthError(error.message, 'login'));
            } else {
                ...
                if (error) setAuthError(friendlyAuthError(error.message, 'signup'));
                else setVerifyEmail(true);
            }
        } catch (e: any) {
            setAuthError(friendlyAuthError(e?.message ?? 'Something went wrong.', mode));
        }
        setLoading(false);
    }
                            ...
                            <View>
                                ...
                                <View style={{ gap: 14 }}>
                                    ...
                                    <View style={{ position: 'relative' }}>
                                        ...
                                        <TextInput
                                            value={email}
                                            onChangeText={(t) => { setEmail(t); if (authError) setAuthError(null); }}
                                            placeholder="Email"
                                            placeholderTextColor={C.textDim}
                                            autoCapitalize="none"
                                    ...
                                    <View style={{ position: 'relative' }}>
                                        ...
                                        <TextInput
                                            value={password}
                                            onChangeText={(t) => { setPassword(t); if (authError) setAuthError(null); }}
